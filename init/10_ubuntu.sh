...
cat <<EOF
This installation of Ubuntu can be updated and a bunch of packages can be installed.  Should we?
This will be skipped if "Y" isn't pressed within the next 15 seconds.
EOF
read -N 1 -t 15 -p "Update Ubuntu? [y/N] " update_ubuntu; echo
if [[ "$update_ubuntu" =~ [Yy] ]]; then
